...
object Html4Component extends BaseComponent {
  ...
  def apply[A, B, C, D](root: HTMLElement,
    ...
    NodesComponentController { c =>
      val nodes = addHtml(c.parent.getOrElse(root))(m, formatter(rv1.v, rv2.v, rv3.v, rv4.v)).orDummyNode
      val wis = List(
  ...
  def apply[A, B, C, D, E](rv1: Reactive[A],
                           ...
                           rv3: Reactive[C],
                           rv4: Reactive[D])
                          (formatter: (A, B, C, D) => HTML): (HTMLElement, AddMethod) => NodesComponentController = {
